...
import abc_dsl_tools
import yaml_tools
import abc_dsl_tools_assembled
...
def get_prompt_template(config, **kwargs):
    if config[SPECIFICATION_TYPE] == SpecificationType.abc:
        return abc_dsl_tools.get_prompt_template(config[MAIN_WORKFLOW], config[MAIN_WORKFLOW_PACKAGE], **config.get(PROMPT_KWARGS, {}), **kwargs)
    ...
    elif config[SPECIFICATION_TYPE] == SpecificationType.abc_ASSEMBLED:
        return abc_dsl_tools_assembled.get_prompt_template(config[MAIN_WORKFLOW], **config.get(PROMPT_KWARGS, {}), **kwargs)
...
def get_specification_tools(config, project_dir: Path):
    if config[SPECIFICATION_TYPE] == SpecificationType.abc:
        return abc_dsl_tools.DSLWorkflowSpecificationTool(project_dir / config[SPECIFICATION_FOLDER])
    elif config[SPECIFICATION_TYPE] == SpecificationType.YAML:
        return yaml_tools.YAMLWorkflowSpecificationTool(project_dir / config[SPECIFICATION_FOLDER])
    elif config[SPECIFICATION_TYPE] == SpecificationType.abc_ASSEMBLED:
        return abc_dsl_tools_assembled.DSLAssembledWorkflowSpecificationTool(project_dir / config[SPECIFICATION_FOLDER])
    raise ValueError("Unsupported specification type: " + config[SPECIFICATION_TYPE])
