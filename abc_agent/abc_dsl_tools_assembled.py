...
class DSLAssembledWorkflowSpecificationTool(BaseTool):
    ...
    def _run(self, workflow_name: str, run_manager=None) -> str:
        workflow_path = self.specification_folder / f"{workflow_name}.abc"
        if not workflow_path.exists():
            available_workflows = [workflow.stem for workflow in self.specification_folder.glob("*.abc")]
...
def list_workflows(specification_folder):
    workflows = [workflow.stem for workflow in specification_folder.glob("*.abc")]
    return workflows
