...
class DSLWorkflowSpecificationTool(BaseTool):
    ...
    def _run(self, package_name: str, workflow_name: str, run_manager=None) -> str:
        ...
        workflow_path = package_path / f"{workflow_name}.abc"
        if not workflow_path.exists():
            available_workflows = [workflow.stem for workflow in package_path.glob("*.abc")]
...
def list_workflows(specification_folder):
    ...
    workflows = [f"{package}.{workflow.stem}" for package in packages for workflow in package.glob("*.abc")]
    return workflows
